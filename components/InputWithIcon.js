            ...
            <div className={`relative rounded-lg border ${error ? 'border-red-300' : isDisabled ? 'border-gray-200 bg-gray-50' : 'border-gray-300 hover:border-blue-400 focus-within:border-blue-500'} transition-colors ${className}`}>
                {Icon && (
                    <div className="absolute left-3 top-1/2 -translate-y-1/2">
                        <Icon className={`w-4.5 h-4.5 ${isDisabled ? 'text-gray-400' : 'text-blue-500'}`} />
                    </div>
                )}
                <input
                    type={type}
                    ...
                    onChange={onChange}
                    disabled={isDisabled}
                    className={`w-full ${Icon ? 'pl-10' : 'pl-3.5'} pr-3.5 py-2.5 focus:border-blue-500 bg-transparent outline-none ${isDisabled ? 'text-gray-500' : ''} focus:ring-2 focus:ring-blue-500/20 text-sm`}
